...
def save_ckpt(output_dir, args, step, train_size, model, optimizer, losses):
    """Save checkpoint"""
    if args.no_save:
        return
...
def main():
    ...
    if args.dataset == "coco2017":
        ...
        cfg.TRAIN.DATASETS = (args.dataset+'_train',)
        cfg.MODEL.NUM_CLASSES = 21
    ...
    for name, module in LCRNet_model.named_modules():
    ...
    nonbias_param_names = []
    nograd_param_names = []
    for key, value in LCRNet_model.named_parameters():
    ...
    if args.load_ckpt:
        ...
        if load_name[-4:]=='.pkl':
            with open(load_name, 'rb') as fid:
                model = pickle.load(fid)
            checkpoint=model['model']
            net_utils.load_ckpt(LCRNet_model, checkpoint)
        else:
            checkpoint = torch.load(load_name, map_location=lambda storage, loc: storage)
            net_utils.load_ckpt(LCRNet_model, checkpoint['model'])
    ...
    if args.load_detectron:  #TODO resume for detectron weights (load sgd momentum values)
        ...
        load_detectron_weight(LCRNet_model, args.load_detectron)

    lr = optimizer.param_groups[0]['lr']  # lr of non-bias parameters, for commmand line outputs.
    #lr=0.000001
    LCRNet_model = mynn.DataParallel(LCRNet_model, cpu_keywords=['im_info', 'roidb'],
                                 minibatch=True)
    ...
    try:
        ...
        for step in range(args.start_step, cfg.SOLVER.MAX_ITER):
            ...
            for inner_iter in range(args.iter_size):
                ...
                net_outputs = LCRNet_model(**input_data)
            ...
            if (step+1) % CHECKPOINT_PERIOD == 0:
                save_ckpt(output_dir, args, step, train_size, LCRNet_model, optimizer, losses)
