...
def extend_with_flipped_entries(roidb, dataset):
    ...
    for entry in roidb:
        ...
        poses = entry['poses'].copy()
        oldx2d_p = poses[:, :13].copy()
        oldx3d_p = poses[:, 26:39].copy()
        poses[:, :13] = width - oldx2d - 1
        poses[:, 26:39] = width - oldx3d - 1
        
        assert (boxes[:, 2] >= boxes[:, 0]).all()
        flipped_entry = {}
        dont_copy = ('boxes', 'poses', 'segms', 'gt_keypoints', 'flipped')
        for k, v in entry.items():
            if k not in dont_copy:
                flipped_entry[k] = v
        flipped_entry['boxes'] = boxes
        flipped_entry['poses'] = poses
...
def add_pose_regression_targets(roidb):
    ...
    for entry in roidb:
        entry['pose_targets'] = _compute_pose_targets(entry)
...
def pose_transform_inv(ex_rois, gt_rois, BBOX_REG_WEIGHTS):
    return gt_rois-ex_rois
    
        
def _compute_pose_targets(entry):
    #print('Computing pose target (TODO)')
    """Compute bounding-box regression targets for an image."""
    # Indices of ground-truth ROIs
    rois = entry['boxes']
    rois_pose = entry['poses']
    
    overlaps = entry['max_overlaps']
    labels = entry['max_classes']
    gt_inds = np.where((entry['gt_classes'] > 0) & (entry['is_crowd'] == 0))[0]
    # Targets has format (class, tx, ty, tw, th) for pose (class, tx1,tx2,...tz13)
    targets = np.zeros((rois_pose.shape[0], 66), dtype=np.float32)
    if len(gt_inds) == 0:
        # Bail if the image has no ground-truth ROIs
        return targets
    ...
    # this will be the ex ROI's gt target
    gt_assignment = ex_gt_overlaps.argmax(axis=1)
    gt_rois = rois_pose[gt_inds[gt_assignment], :]
    ex_rois = rois_pose[ex_inds, :]
    # Use class "1" for all boxes if using class_agnostic_bbox_reg
    targets[ex_inds, 0] = (
        1 if cfg.MODEL.CLS_AGNOSTIC_BBOX_REG else labels[ex_inds])
    targets[ex_inds, 1:] = pose_transform_inv(
        ex_rois, gt_rois, cfg.MODEL.BBOX_REG_WEIGHTS)
