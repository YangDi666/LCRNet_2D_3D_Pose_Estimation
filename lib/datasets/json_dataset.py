...
class JsonDataset(object):
    ...
    def __init__(self, name):
        ...
        categories=['anchor1', 'anchor2', 'anchor3', 'anchor4', 'anchor5', 'anchor6', 'anchor7', 'anchor8', 'anchor9', 'anchor10', 'anchor11', 'anchor12', 'anchor13', 'anchor14', 'anchor15', 'anchor16', 'anchor17', 'anchor18', 'anchor19', 'anchor20' ]
        self.category_to_id_map = dict(zip(categories, category_ids))
        self.classes = ['__background__'] + categories
        print('categories:', self.category_to_id_map)
    ...
    def valid_cached_keys(self):
        ...
        """
        keys = ['boxes', 'poses', 'segms', 'gt_classes', 'seg_areas', 'gt_overlaps',
    ...
    def _prep_roidb_entry(self, entry):
        ...
        entry['poses'] = np.empty((0, 13*NT), dtype=np.float32)
    ...
    def _add_gt_annotations(self, entry):
        ...
        for obj in objs:
            ...
            if obj['area'] > 0 and x2 > x1 and y2 > y1 and len(obj['pose'])==65:
                #print(obj)
        ...
        poses = np.zeros((num_valid_objs, 13*NT), dtype=entry['poses'].dtype)
        ...
        for ix, obj in enumerate(valid_objs):
            ...
            poses[ix, :] = obj['pose']
            cls=_get_class_for_person(obj, poses[ix,:], anchor_poses)
            #classes[cls-1]+=1
            gt_classes[ix] = cls
    ...
    def _add_gt_from_cache(self, roidb, cache_filepath):
        ...
        for entry, cached_entry in zip(roidb, cached_roidb):      
            ...
            boxes, poses, segms, gt_classes, seg_areas, gt_overlaps, is_crowd, \
    ...
    def _init_keypoints(self):
        ...
        # Thus far only the 'person' catego"ry has keypoints
    ...
    def _get_gt_keypoints(self, obj):
        ...
        gt_kps = np.zeros((3, self.num_keypoints), dtype=np.int32)
        for i in range(12):
            gt_kps[0, i] = x[16-i]
            gt_kps[1, i] = y[16-i]
            gt_kps[2, i] = v[16-i]
...
def add_proposals(roidb, rois, scales, crowd_thresh):
    ...
    for i in range(len(roidb)):
        ...
        pose_list.append(rois[idx, 1:] * inv_im_scale)
        
    _merge_proposal_boxes_into_roidb(roidb, box_list)
    #_merge_proposal_poses_into_roidb(roidb, pose_list)
...
def _merge_proposal_boxes_into_roidb(roidb, box_list):
    ...
    for i, entry in enumerate(roidb):
        ...
        entry['poses'] = np.append(
            entry['poses'],
            np.zeros((num_boxes, 13*NT), dtype=entry['poses'].dtype).astype(entry['poses'].dtype, copy=False),
            axis=0
        )
...
def _get_class_for_person(obj, gt_kps, anchor_poses):
   
    gt_pose=gt_kps.astype(np.float32)
    box=np.array(obj['clean_bbox'], dtype=np.float32).reshape(1,4)
    box_size = box[:,2:4]-box[:,0:2]

    offset = np.concatenate( (box[:,:2], np.zeros((1,3),dtype=np.float32)), axis=1) # x,y top-left corner for each box
    scale = np.concatenate( (box_size[:,:2], np.ones((1,3),dtype=np.float32)), axis=1) # width, height for each box
    ...
    gt_pose =(gt_pose-offset_poses)/scale_poses
      
    gt_pose=np.tile(gt_pose, (anchor_poses.shape[0],1))
    dis=(gt_pose[:,:13*NT]-anchor_poses[:,:13*NT])**2
    dis_total=np.sqrt(dis.sum(axis=1))
    class_min=np.argmin(dis_total)+1
