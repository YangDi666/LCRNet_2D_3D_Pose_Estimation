...
class lcrnet_outputs(nn.Module):
    def __init__(self, dim_in, J):
        ...
        pred_per_joint = NT*J # 5 for 3D+2D
...
def lcrnet_losses(cls_score, pose_pred, label_int32, pose_targets,
                     pose_inside_weights, pose_outside_weights):
    device_id = cls_score.get_device()
    rois_label = Variable(torch.from_numpy(label_int32.astype('int64'))).cuda(device_id)
    loss_cls = F.cross_entropy(cls_score, rois_label)

    pose_targets = Variable(torch.from_numpy(pose_targets)).cuda(device_id)
    pose_inside_weights = Variable(torch.from_numpy(pose_inside_weights)).cuda(device_id)
    pose_outside_weights = Variable(torch.from_numpy(pose_outside_weights)).cuda(device_id)
    loss_pose = net_utils.smooth_l1_loss(
        pose_pred, pose_targets, pose_inside_weights, pose_outside_weights)

    # class accuracy
    cls_preds = cls_score.max(dim=1)[1].type_as(rois_label)
    accuracy_cls = cls_preds.eq(rois_label).float().mean(dim=0)

    return loss_cls, loss_pose, accuracy_cls
