...
import modeling.lcrnet_heads as lcrnet_heads
...
class LCRNet(nn.Module):
    def __init__(self, njoints=13):
        ...
        if not cfg.MODEL.RPN_ONLY:
            ...
            self.Pose_Outs = lcrnet_heads.lcrnet_outputs(
                self.Box_Head.dim_out, njoints)
    ...
    def _forward(self, data, im_info, roidb=None, **rpn_kwargs):
        ...
        if self.training:
           # print(rpn_ret.keys())
            ...
            loss_cls, loss_pose, accuracy_cls = lcrnet_heads.lcrnet_losses(
                cls_score, pose_pred, rpn_ret['labels_int32'], rpn_ret['pose_targets'],
                rpn_ret['pose_inside_weights'], rpn_ret['pose_outside_weights'])
            return_dict['losses']['loss_cls'] = loss_cls
            return_dict['losses']['loss_pose'] = loss_pose
